...
**1.1. Entidad `Tenant` y Puerto de Repositorio (Core)**
    *   **Acción IA Nx:**
        *   Asegurar que exista `libs/core/src/tenancy/entities/tenant.entity.ts`.
        *   Asegurar que exista `libs/core/src/tenancy/ports/tenant-repository.port.ts`.
        *   Asegurar que exista `libs/shared/src/enums/internal-tenant-status.enum.ts` (si no se definió ya).
    *   **Archivo: `libs/shared/src/enums/internal-tenant-status.enum.ts`**
        ```typescript
        ...
        ```
    *   **Archivo: `libs/core/src/tenancy/entities/tenant.entity.ts`**
        ```typescript
...
**1.2. Puerto de Configuración Segura del Tenant (Core)**
    *   **Acción IA Nx:** Crear `libs/core/src/tenancy/ports/tenant-config-repository.port.ts`.
    *   **Archivo: `libs/core/src/tenancy/ports/tenant-config-repository.port.ts`**
        ```typescript
...
**1.3. Implementación de Repositorios de Tenancy (Infraestructura)**
    *   **Acción IA Nx:**
        *   Crear librería `libs/infrastructure/src/tenancy-persistence/` y `libs/infrastructure/src/security/`.
        *   Configurar `PrismaService` (o un `PlatformPrismaService`) para la DB de plataforma.
    *   **Archivo: `libs/infrastructure/src/security/ports/encryption-service.port.ts` (Mover `IEncryptionServicePort` a `core` si es más genérico)**
        ```typescript
        ...
        ```
    *   **Archivo: `libs/infrastructure/src/security/services/aes-encryption.service.ts`**
        ```typescript
